...
'''
# Separated by genre
genre = rolling_stone.groupby('Album Genre', as_index=False).sum()

#Show 2020 - 2003 differential by genre
diff_genre = genre.sort_values(by='2020-2003 Differential', ascending=True)
...
'''
# How many love songs in the top 100 each decade?

spotify_most_popular = rolling_stone.groupby('Spotify Popularity', as_index=False).sum()
ordered = spotify_most_popular.sort_values(by='Spotify Popularity', ascending=False)
...
'''
Average age at top 500 album over time ( by release year)
'''

# print(rolling_stone.columns)
# group_age = rolling_stone.sort_values(by='Avg. Age at Top 500 Album', ascending=False)
# name_age = group_age[['Clean Name', 'Avg. Age at Top 500 Album']]
#
#
# print(name_age.head())
#
# bins = list(range(1900, 2030, 10))  # Decades from 1900 to 2020s
# labels = [f"{decade}s" for decade in range(1900, 2020, 10)]
#
# decades = rolling_stone.groupby(pd.cut(rolling_stone['Release Year'], bins=bins, labels=labels))
#
# # Display first few rows from each group
# print(decades.apply(lambda x: x.head()))
#
