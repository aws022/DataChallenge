...
def plot_top_artists_prediction(artist_metrics, output_dir='output'):
    """
    Create a refined visualization of predicted artist success for 2025-2026.
    """
    ...
    # Calculate prediction score (model logic verified: weighted sum with total weight = 1.0)
    artist_metrics['prediction_score'] = (
        artist_metrics['max_weeks_on_chart'] * 0.6 +
        artist_metrics['avg_weeks_on_chart'] * 0.3 +
        artist_metrics['avg_spotify_popularity'] * 0.1
    ).round(1)
    ...
    # Get top 10 artists and sort in descending order (highest prediction score at the top)
    ...
    plt.title('Top 10 Artists: Predicted Billboard Hot 100 Success (2025-2026)',
             fontsize=14,
             color=colors['text'],
             pad=20,
             fontweight='bold')
    ...
    ax.text(0.5, 1.08,
            'Based on Historical Performance and Current Popularity',
    ...
    plt.savefig(os.path.join(output_dir, 'artist_success_prediction.png'),
                dpi=300,
                bbox_inches='tight',
                facecolor='white')
    plt.close()
